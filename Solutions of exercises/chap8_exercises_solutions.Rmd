...
```{r}
#Logistic regression
model <- glm(call~education+yearsexp+race+sex+eoe+wanted, family = binomial(link = 'logit'), data = data)
summary(model)
```
...
log_reg_fun <- function(dat){
  ...
  mod <- glm(call~education+yearsexp+race+sex+eoe+wanted, dat, family = 'binomial')
  summ <- summary(mod)
