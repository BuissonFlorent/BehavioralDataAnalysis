...
```{r}
### Figure 10-3. Visual representation of moderation ###
viz_fun_10.3 <- function(dat){
...
viz_fun_10.3(hist_data)
```

## Non-linearities ##

```{r}
### NEED TO TRANSFER THAT DATA TO A CSV FILE AND READ FROM IT INSTEAD ###
...
```{r}
### Figure 10-14. Non-linear relationships between variables ###
viz_fun_10.14 <- function(dat){
...
viz_fun_10.14(nonlin_data)
```

```{r}
### Figure 10-15. Linear (dashed) and quadratic (solid) lines of best fit ###
viz_fun_10.15 <- function(dat){
...
```{r}
### Figure 10-23. Moderated moderation across different age groups ###
viz_fun_10.23 <- function(dat){
  summary_dat <- dat %>%
...
mod_summ10k <- mod_boot_fun(hist_data, B = 1e3, N = 1e5)

### Figure 10-25. Distribution of bootstrapped values for interaction coefficient (1k samples of 10k rows) ###
viz_fun_10.25 <- function(dat){
  ggplot(dat, aes(x=coeff_i)) + geom_histogram() + xlim(c(19.5,22.5)) +
    geom_vline(xintercept = 20.985, col='red') + xlab("beta_pc")
}
viz_fun_10.25(mod_summ10k)
```
...
### Figure 10-26. Distribution of bootstrapped values for interaction coefficient (1k samples of 200k rows) ###
viz_fun_10.26 <- function(dat){
  ggplot(mod_summ, aes(x=coeff_i)) + geom_histogram() + xlim(c(19.5,22.5)) +
    geom_vline(xintercept = 20.985, col='red') + xlab("beta_pc")
}
viz_fun_10.26(mod_summ200k)
```
...
```{r}
### Figure 10-27. Sample of 1,000 data points and regression lines with a play area (full dots, solid line) and without (crosses, dashed line), without moderation term ###
viz_fun_10.27 <- function(dat){
...
viz_fun_10.27(extract_dat)
```

```{r}
### Figure 10-28. Sample of 1,000 data points and regression lines with a play area (full dots, solid line) and without (crosses, dashed line), with moderation term ###
viz_fun_10.28 <- function(dat){
  ggplot(extract_dat, aes(x=age, y=duration, col=play_area)) + 
    geom_point(aes(shape = play_area), alpha = 0.8) +
    scale_shape_manual(values=c(4, 16)) +
