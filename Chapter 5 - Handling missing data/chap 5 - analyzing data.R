#################################
##### This script analyzes the simulated data used in chapter 5, 
...
#Loading the data
complete_data <- read_csv("chap5-complete_data.csv") %>%
  mutate(state = factor(state, levels = c("A", "B", "C")))%>%
  mutate(gender = factor(gender, levels = c("M", "F")))
available_data <- read_csv("chap5-available_data.csv") %>%
  mutate(state = factor(state, levels = c("A", "B", "C"))) %>%
  mutate(gender = factor(gender, levels = c("M", "F")))
available_data_supp <- read_csv("chap5-available_data_supp.csv")
...
tacoma <- read_csv("chap5-tacoma.csv")
tampa <- read_csv("chap5-tampa.csv")
