...
This is the [R Markdown](http://rmarkdown.rstudio.com) Notebook for chapter 7: Experimental design 1 - the basics
...
library(boot) # For traditional bootstrap
...
```{r}
# Figure 7-3 Power simulations for various sample sizes
viz_fun7.3 <- function(){
  power_res <- tibble(
...
```{r}
#Optimized functions necessary to build figure 7-4
library(Rfast)
library(doParallel)
...
```{r}
### Figure 7-4. Power simulations for various effect sizes at N = 40,000, 
...
boot_CI_fun(exp_data, metric_fun = log_reg_fun)
```
...
boot_CI_fun(exp_data, diff_prob_fun)
```